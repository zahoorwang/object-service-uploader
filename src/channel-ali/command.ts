...
export function commandify() {
  ...
  oss
    ...
    .addOption(new Option('--access <string>', `the bucket you want to access's root path`).env('OSU_OSS_ACCESS'))
    ...
    .action(async (opts: any) => {
      ...
      const cname = Boolean(options.endpoint) && !new Endpoint().has(options.endpoint);
      ...
      await new Uploader({ ...options, cname })
        .putting({
          files,
          access,
          before() {
